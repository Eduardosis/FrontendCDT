...
const ProviderRepShip = () => {
    ...
    const handleViewRutas = async (envioId) => {
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/ee-ruta/?envio=${envioId}`);
        ...
        <div className="ShowReports">
            ...
            <Container className="pt-5">
                <Row className="justify-content-center">
                    <Col xs={12}>
                        <h2 className="text-center">Reportes de Envío</h2>
                            ...
                            <tbody>
                                {reportesEnvio.filter(reporte => ordenes[reporte.ordenenvio]).map((reporte) => (
                                    <tr key={reporte.idreporteenv}>
                                        ...
                                        <td>{envios[reporte.envio] || 'Hora no encontrada'}</td>
